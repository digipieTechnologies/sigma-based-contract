...
/// @title This is official MAGNET Token contract ERC20 used in the Based Sigma platform
/// @author Based Sigma Team
...
contract BasedSigmaToken is ERC20, Ownable {
    ...
    uint8 public platformPercentage = 50;
    uint8 public stakerPercentage = 50;
    uint8 public lpPercentage = 0;
    ...
    constructor() ERC20("Based Sigma", "SIGMA") Ownable(msg.sender) {
        super._update(address(0), msg.sender, (10_000_000 * 10 ** 18));

        address uniswapV2Router02Address = 0x4752ba5DBc23f44D87826276BF6Fd6b1C372aD24;
    ...
    function _update(
        ...
        if (fromState.isMarketPair || toState.isMarketPair) {
            require(
                ...
                "BasedSigma: Trading not open yet"
            );
        }
    ...
    function distributeTax() external lockTheSwap {
        require(
            ...
            "BasedSigma: owner/contract only"
        );
        ...
        uint256 lpCut = 0;
        if (lpPercentage > 0) {
            lpCut = (contractBalance * lpPercentage) / 100;
        }

        require(
            (platformCut + stakerCut + lpCut) <= balanceOf(address(this)),
            "BasedSigma: Can't distribute the funds"
        );
    ...
    function _handleLiquidityAndPlatformCut(
        uint256 platformCut,
        uint256 lpCut
    ) private {
        ...
        require(success, "BasedSigma: Failed to send ETH to tax address");
    ...
    function _swapTokensForEth(uint256 tokenAmount_) private {
        ...
        require(
            tokenAmount_ > 0,
            "BasedSigma: Token amount less then 0 for token swap"
        );
    ...
    function updateStakingContracts(
        ...
        require(
            ...
            "BasedSigma: No of address and No of Percentages doesn't match!"
        );
        require(
            stakingContracts_.length <= 10,
            "BasedSigma: Max 10 staking contracts allowed!"
        );
        ...
        for (uint256 i = 0; i < stakingContracts_.length; i++) {
            require(
                ...
                "BasedSigma: percentage should be between  0 to 100!"
            );
            require(
                stakingContracts_[i] != address(0),
                "BasedSigma: Staking contract may not be 0x0"
            );
        ...
        require(
            totalPercent == 100,
            "BasedSigma: Total Percentage should be 100."
        );
    ...
    function openTrading() external onlyOwner {
        require(isTradingOpen == false, "BasedSigma: Trading already open");
    ...
    function updateUniswapRouter(address newRouter) external onlyOwner {
        ...
        require(oldAddress != newRouter, "BasedSigma: Address already set");
    ...
    function setTaxExempt(address account, bool isExempt) external onlyOwner {
        require(account != address(this), "BasedSigma: Can't change contract");
    ...
    function setTaxAmount(
        ...
        require(_isTaxRenounced == false, "BasedSigma: Tax is renounced!");
        require(
            newBuyTax_ <= 10 && newSellTax_ <= 10,
            "BasedSigma: Tax Should be less then 10!"
        );
        buyTax = newBuyTax_;
        sellTax = newSellTax_;
    ...
    function setMarketPair(address account, bool value) public onlyOwner {
        require(account != address(this), "BasedSigma: cant change contract");
        WalletState memory state = walletStates[account];
    ...
    function setDistributionPercentage(
        ...
        require(
            ...
            "BasedSigma: Percentage should sum to 100!"
        );
    ...
    function changeTaxDistributionThreshold(
        ...
        require(
            ...
            "BasedSigma: Threshold can't be 0"
        );
    ...
    function rescueStuckETH(uint256 amount_) external onlyOwner {
        require(amount_ > 0, "BasedSigma: amount can't be 0");
