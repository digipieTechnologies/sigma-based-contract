...
contract BasedSigmaStaking is Ownable {
    using SafeERC20 for IERC20;
    address public immutable basedSigmaTokenAddress;
    ...
    constructor(address _basedSigmaTokenAddress) Ownable(msg.sender) {
        basedSigmaTokenAddress = _basedSigmaTokenAddress;
    }
    ...
    function stakeTokens(uint256 stakeAmount_) external {
        require(
            openToDeposits == true,
            "BasedSigmaStaking: This contract is no longer accepting new stakes. Users can still withdraw."
        );
        require(
            stakeAmount_ >= minimumDepositAmount,
            "BasedSigmaStaking: Please stake above Minimum Deposit Amount!"
        );
        uint256 tvl = IERC20(basedSigmaTokenAddress).balanceOf(address(this));
        ...
        IERC20(basedSigmaTokenAddress).transferFrom(
            msg.sender,
            address(this),
            stakeAmount_
        );
    ...
    function unstakeAll() external returns (uint256) {
        ...
        require(shares > 0, "BasedSigmaStaking: No shares to unstake!");
        uint256 tvl = IERC20(basedSigmaTokenAddress).balanceOf(address(this));
        ...
        IERC20(basedSigmaTokenAddress).safeTransfer(msg.sender, value);
    ...
    function unstakeTokens(uint256 unstakeAmount_) public {
        ...
        require(
            ...
            "BasedSigmaStaking: Not enough staked!"
        );
        ...
        IERC20(basedSigmaTokenAddress).safeTransfer(msg.sender, unstakeAmount_);
